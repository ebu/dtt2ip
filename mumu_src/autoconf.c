...
/** Alex
 * Get the values from the unicast_handle_message() function
*/
unicast_parameters_t *unicastParamAlex;
unicast_client_t *clientAlex;
mumudvb_channel_t *channelsAlex;
int numChannelsAlex;
strength_parameters_t *strengthParamsAlex;
auto_p_t *autopAlex;
void *campAlex;
void *scamvAlex;
eit_packet_t *eitPacketsAlex;
int alexAgainCalled = 0;
char *substringAlex = NULL;
int requestedSidAlex;
char *ip_ownerAlex = NULL;
int numClientsAlex = 1;

void alexAgain(unicast_parameters_t *unicast_vars,
		...
		int requestedSid)
{
	unicastParamAlex = unicast_vars;
	clientAlex = client;
	substringAlex = strdup(client->buffer);
	channelsAlex = channels;
	numChannelsAlex = number_of_channels;
	strengthParamsAlex = strengthparams;
	autopAlex = auto_p;
	campAlex = cam_p;
	scamvAlex = scam_vars;
	eitPacketsAlex = eit_packets;
	alexAgainCalled = 1;
	requestedSidAlex = requestedSid;
	//printf("Alex autoconfig alexAgain called\n");
	//printf("Alex autoconfig alexAgain %s\n", clientAlex->buffer);
}
		...
		if(auto_p->pat_all_sections_seen)
		{
			...
			if(channel_updated)
			{
				...
				if(!channel_left)
				{
					...
					int retuneAlex = 1; 
					int iRet;
					//printf("Alex autoconfig alexAgainCalled = %d\n", alexAgainCalled);
					...
					if (alexAgainCalled) 
					{
						alexAgainCalled = 0;
						printf("Alex autoconfig alexAgain %s\n", substringAlex);
						printf("Alex autoconfig alexAgain %d\n", requestedSidAlex);
						clientAlex->buffer = strdup(substringAlex);
						iRet=unicast_handle_message(unicastParamAlex, clientAlex, channelsAlex, numChannelsAlex, strengthParamsAlex, autopAlex, campAlex, scamvAlex, eitPacketsAlex, retuneAlex, requestedSidAlex, ip_ownerAlex, numClientsAlex);
						if (iRet == -3)
							printf("Alex autoconfig unicast_handle_message ok\n");
						else
							printf("Alex autoconfig unicast_handle_message not ok\n");
					}	
				}
			}
		}
