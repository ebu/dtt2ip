...
int channel_add_unicast_client(unicast_client_t *client,mumudvb_channel_t *channel, int filename_port, int freq, int client_number);
		...
		if((unicast_vars->pfds[actual_fd].revents&POLLIN)||(unicast_vars->pfds[actual_fd].revents&POLLPRI))
		{
			...
			{	
				...
				if(tempClient!=NULL)
				{
					...
					printf("Alex unicast_accept_connection 1 %s\n", inet_ntoa(unicast_vars->clients->SocketAddr.sin_addr));
			...
			else if(unicast_vars->fd_info[actual_fd].type==UNICAST_CLIENT)
			{
				...
				//Alex
				printf("Alex unicast_accept_connection 2 %s\n", inet_ntoa(unicast_vars->clients->SocketAddr.sin_addr));
				int retuneAlex = 0;
				int requested_sid = 0;
				log_message( log_module, MSG_FLOOD,"New message for socket %d\n", unicast_vars->pfds[actual_fd].fd);
				iRet=unicast_handle_message(unicast_vars,unicast_vars->fd_info[actual_fd].client, channels, number_of_channels, strengthparams, auto_p, cam_p, scam_vars,eit_packets, retuneAlex, requested_sid);
...
unicast_client_t *unicast_accept_connection(unicast_parameters_t *unicast_vars, int socketIn)
{
	...
	//Alex
	printf("Alex Maximum number of clients available : %d\n", unicast_vars->max_clients);
...
{
	//Alex
	printf("Alex Unicast: close connetion\n");
	int actual_fd;
	actual_fd=0;
		...
		int requested_sid)
{
	...
	if(((client->buffersize-client->bufferpos)<RECV_BUFFER_MULTIPLE) && (retuneAlex == 0))
	{
	...
	//Alex
	//printf("Alex unicast_handle_message 11 %s\n",client->buffer);
	if (retuneAlex == 0)
	{
	...
	//printf("Alex Unicast: client buffer 2\n");
	//printf("Alex unicast_handle_message 2 %s\n",client->buffer);
	if((received_len>0) && (retuneAlex ==0))
	{
	...
	//printf("Alex unicast_handle_message 22 %s\n",client->buffer);
	if((received_len==-1) && (retuneAlex ==0))
	{
	...
	//printf("Alex Unicast: client buffer 2: %s\n", client->buffer)
	if ((retuneAlex == 0) && (retuneAlex ==0))
	...
	//printf("Alex Unicast: client buffer 3: %s\n", client->buffer);
	printf("Alex unicast_handle_message 1 %s\n", inet_ntoa(unicast_vars->clients->SocketAddr.sin_addr));
	printf("Alex unicast_handle_message 2 %d\n", unicast_vars->portOut);
	if(strlen(client->buffer)>5 && strstr(client->buffer, "\n\r\n\0") && retuneAlex == 0)
	{
		...
		//Alex
		tune_p_t tune_p;
		char strAlex[]="GET /bysid/1281/byfreq/770000 HTTP/1.1\nHost: 192.168.1.115:4028\nAccept: */*\nUser-Agent: VLC/3.0.0-git LibVLC/3.0.0-git\nRange: bytes=0-";
		///Alex
		log_message( log_module, MSG_FLOOD,"End of HTTP request, we parse it\n");
		//printf("Alex Unicast: client buffer 4: %s\n", client->buffer);
		if(strstr(client->buffer,"GET ")==client->buffer || strstr(client->buffer,"HEAD ")==client->buffer)
		{
			//to implement :
			//Information ???
			//GET /monitor/???
			//printf("Alex unicast_handle_message GET/HEAD\n"); 
			//substringAlex = &(client->buffer[strlen(client->buffer)-(strlen(client->buffer)-4)]);
			//printf("Alex %s\n", substringAlex);
			//printf("Alex %s\n", client->buffer);
			///Alex
			pos=4;

			// Recover the original GET/HEAD
			if (strstr(client->buffer,"HEAD ")==client->buffer)
			{
				substring2 = strdup(strAlex);
				substring3 = strdup(strAlex);
			}
			else
			{
				substring2 = strdup(client->buffer);
				substring3 = strdup(client->buffer);
			}
			...
			//Alex GET /bysid/sid/byfreq/freq
			//GET /bysid/sid
			else if(strstr(client->buffer +pos ,"/bysid/")==(client->buffer +pos) || strstr(strAlex +pos ,"/bysid/")==(strAlex +pos))
			{	
				if(client->chan_ptr!=NULL)
				{	
					log_message( log_module, MSG_INFO,"A channel (%s) is already streamed to this client, it shouldn't ask for a new one without closing the connection, error 501\n",client->chan_ptr->name);
				...
				pos+=strlen("/bysid/");
				substring = strtok (substring2+pos, "/");			
				if(substring == NULL)
				{
					err404=1;
				}
				else
				{
					...
					if (strstr(client->buffer,"HEAD ")==client->buffer)
						...
						substring2 = client->buffer;
					// Get the frequency requested, for retuning purpose
					if (strstr(client->buffer +pos ,"/byfreq/")==(client->buffer +pos) || strstr(strAlex +pos ,"/byfreq/")==(strAlex +pos))
					{
						pos+=strlen("/byfreq/");
						substring = strtok (substring2+pos, " ");
						
						// Recover the original GET/HEAD
						client->buffer = strdup(substring3);
						int requested_freq;
						requested_freq = atoi(substring);
						if (requested_freq != tuned_freq)
						{	
							char *ip_owner = NULL;
							char *ip_client = NULL;

							ip_owner = inet_ntoa(unicast_vars->clients->SocketAddr.sin_addr);
							ip_client = inet_ntoa(client->SocketAddr.sin_addr);

							if (unicast_vars->client_number >= 2)
							{
								printf("Alex retune more then 1 user\n");
								err404=1;
								requested_channel=0;	
							}
							else
							{
								if (strstr(ip_owner, ip_client) == ip_owner)
								{
									printf("Alex retune owener\n");
									tuned_freq = requested_freq;
									printf("Alex retune\n");
									init_tune_v(&tune_p);
									tune_p.freq = requested_freq;
									iRet = tune_it(3,&tune_p);
									retuneAlex = 0;
									alexAgain(unicast_vars, client, channels, number_of_channels, strengthparams, auto_p, cam_p, scam_vars,eit_packets, retuneAlex, requested_sid);
								}
								else
								{
									printf("Alex retune not a owner\n");
									err404=1;
									requested_channel=0;
								}
							}
			...
			if(requested_channel)
			{
				printf("Alex 7  requested_sid= %d\n", requested_sid);
				//printf("Alex 7  requested_sid= %s\n", unicast_vars->portOut);
				if(!channel_add_unicast_client(client,&channels[requested_channel-1], unicast_vars->portOut, tuned_freq, unicast_vars->client_number))
	...
	if (retuneAlex)
	{
		...
		{
			//printf("Alex 6\n");
			printf("Alex 7 service_id %d", channels[current_channel].service_id);
			if(channels[current_channel].service_id == requested_sid)
				requested_channel=current_channel+1;
		}
		printf("Alex 7  requested_channel= %d\n", requested_channel);
		if(requested_channel)
			log_message( log_module, MSG_DEBUG,"Channel by service id,  service_id %d number %d\n", requested_sid, requested_channel);
		else
		{
