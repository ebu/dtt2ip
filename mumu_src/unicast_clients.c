...
int unicast_del_client(unicast_parameters_t *unicast_vars, unicast_client_t *client)
{
	...
	//Server consider it self as a client. So the counting of connetions(clients) will start from 1. 
	printf("Alex wrote number of clients remaining=%d\n", unicast_vars->client_number);
	if (unicast_vars->client_number == 1)
	{
		...
		// printf("Alex wrote to file= %s\n", filename_port2);
		snprintf(filename, sizeof filename, "%s%d%s", filename_b, unicast_vars->portOut, filename_ending);

		fp = fopen(filename, "w");
		fprintf(fp, "0");
		fclose(fp);
		printf("Alex wrote to file test_mumu0.txt 0\n");
	}
...
int channel_add_unicast_client(unicast_client_t *client,mumudvb_channel_t *channel, int filename_port, int freq, int client_number)
{
	...
	if ((filename_port) && (client_number > 1))
	{
		...
		// printf("Alex wrote to file= %s\n", filename_port2);
		snprintf(filename, sizeof filename, "%s%d%s", filename_b, filename_port, filename_ending);
		printf("Alex wrote to file=%s\n", filename);
		printf("Alex wrote to file test_mumu0.txt %s\n", freq_str);
		fp = fopen(filename, "w");
		fprintf(fp, freq_str);
		fclose(fp);
	}
